define(['directives/directives', 'bootstrap-dialog', 'datatables.net', 'datatables.net-bs', 'datatables.net-select'], function(directives, BootstrapDialog, datatables) {
  'use strict';
  directives.initDirective('workorderRecordTable', ['$timeout', '$compile', 'growl', '$filter',"ngDialog", function($timeout, $compile, growl, $filter,ngDialog) {
    return {
      restrict: 'A',
  ...
  directives.initDirective('workorderRecordPageTable', ['$timeout', '$compile', 'growl', '$filter',"ngDialog", function($timeout, $compile, growl, $filter,ngDialog) {
    return {
      restrict: 'A',
      controller: ['$scope', '$element', '$attrs',
        function($scope, $element, $attrs) {
          ...
          var table;
          var isEditing = false;
          $scope.exportAlertData={reason:""};//导出报警对象
            ...
            table = domMain.DataTable({
              ...
              // },
                ...
                "render": function(data, type, full) {
                  // 返回自定义内容
                  var str = "<div class='btn-group btn-group-sm'>";
                  if(full.taskStatus == 1) {//任务状态是新产生的 才能确认
                    if($scope.menuitems['A08_S05']) {
                      str += "<button id='claim-btn' class='btn btn-primary' ><span class='hidden-xs'> 确认</span></button>";
                    }
                  } else {
                  ...
                  if(full.taskStatus == 1) {//任务状态是新产生的 才能关闭
                    if($scope.menuitems['A07_S05']) {
                      str += "<button id='close-btn' class='btn btn-default' ><span class='hidden-xs'> 关闭</span></button>";
                    }
                  } else {
...
//              }
          ...
          $scope.selectedOrder = function() {
            ...
            if($scope.uniqueJsonArray($scope.confirmActiveAlert,"alarmSource") > 1) {
