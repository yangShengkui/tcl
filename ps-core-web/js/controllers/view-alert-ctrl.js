define(['controllers/controllers', 'bootstrap-dialog'], function(controllers, BootstrapDialog) {
  ...
  controllers.initController('ViewAlertCtrl', ['$scope', '$q', 'workflowService', '$routeParams', 'ngDialog', 'userLoginUIService', 'alertService', 'SwSocket', 'Info',
    'viewFlexService', 'alertManageFlexService', 'growl', 'ticketCategoryService', 'ticketService', 'processService', '$filter', '$timeout', '$location','userUIService',
    function($scope, $q, workflowService, $routeParams, ngDialog, userLoginUIService, alertService, SwSocket, Info, viewFlexService,
      alertManageFlexService, growl, ticketCategoryService, ticketService, processService, $filter, $timeout, $location,userUIService) {
      var columns = [];
      var columsData = [];
      var viewInfo;
      var searchAlertData;
      ...
      $scope.selectedHandler = function() {
        ...
        $scope.pageAlertList.forEach(function(obj) {
          if(obj.selected && obj.state == 0) {//state 是新产生的才可以确认 关闭
      ...
      $scope.selectTicketCategory = function () {
        ...
        $scope.addManualAlert = function(){
          $scope.alertTitleList = [];
          alertManageFlexService.getAlertTitleList({},function (returnObj){
              if(returnObj.code == 0) {
                  $scope.alertTitleList = returnObj.data;
              }
              $scope.alertTitleList.push({"id":"0","label":"其他原因"});
          });
            ngDialog.open({
      ...
      $scope.doAction = function(type, select, callbackFun) {
        if(type == "claim") {
          ...
          var alertInfo = {
            ...
            "resolved": true,
            "clearOut": true,
              "disposeType":$scope.closeAlarmData.disposeType,
            "remark":$scope.closeAlarmData.remark
          };
          alertManageFlexService.sendRecoverAction(alertInfo, function(returnObj) {
            if(returnObj.code == 0) {
              for(var i = columsData.length - 1; i > -1; i--) {
      ...
      if($routeParams.nodeId) {
        ...
        $scope.saveManualAlert = function () {
            $scope.manualAlert.title = $scope.manualAlert.alertReasonData.label;
            $scope.manualAlert.alertCode = $scope.manualAlert.alertReasonData.id;
            if($scope.manualAlert.alertCode=='0'&&($scope.manualAlert.message==undefined||$scope.manualAlert.message=='')){
              growl.warning('请输入其它原因', {});
              return;
          }
            ...
            if ($scope.commitloading) {
                growl.warning("请等待，正在保存人工报警", {});
                return;
            }
            $scope.commitloading = true;
            var ticket = $scope.manualAlert;
            alertManageFlexService.addAlert(ticket,  function(returnObj) {
                $scope.commitloading = false;
                if(returnObj.code == 0) {
                    growl.success("保存人工报警成功", {});
                    $scope.manualAlert={};
                    $scope.$broadcast(Event.ALERTINFOSINIT, {
                        "option": []
                    });
        ...
        //导出报警
        $scope.exportAlertsubmit = function(){
         //  var exportParam= $scope.exportAlertData.week ? $scope.exportAlertData.week.join(",") : '';//导出规则

            alertManageFlexService.exportExcelByWeek(searchAlertData==undefined?{}:searchAlertData,function (returnObj) {//[exportParam]
                if(returnObj.code == 0) {
            ...
            //alert($filter('date')($scope.selectedAlertitem.createTimeFrom , "yyyy-MM-dd"));
        }
    }
